...
| `POST /api/sessions` | `{ machineId, cwd, name?, prompt?, sessionFile? }` → 202 `{ session }` (status `starting`); 404 unknown machine; 400 missing fields or blank `sessionFile`. `sessionFile` resumes that omp session file (`start.sessionFile`, §2) |
...
`{ id, cwd, name?, prompt?, sessionFile?, relayUrl, webUrl, agentDir? }`. `sessionFile` resumes
