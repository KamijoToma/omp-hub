...
| 17 | **Steering messages (2026-09-23, hub session view)**: live hub + real browser on `/s/<id>`; the collab host was a scripted stand-in (fake agent daemon registered the session, fake room host replayed `state`/`entry` frames) since no SDK credentials were involved. Prompt submitted while `isStreaming` → host `prompt` frame, queue bar above the composer shows the text with the "delivered at the next tool call or when the turn ends" hint alongside the vendored `queued ×N` chip; Enter on the empty editor and the bar's Interrupt & send button each sent `abort` → host delivered the queued text as a `collab-prompt` entry and went idle → bar cleared, transcript showed the entry with the `E2E Guest` author badge. Unit coverage in `packages/web/test/steering-queue.test.ts` | ✅ (screenshot reviewed) |
| 18 | **Rewind fix + tree navigation (2026-09-23, feat/web-session-tree)**: worktree hub `:18091` + worktree agent, real SDK session (glm-5.3-flash) with two browser-sent turns. Root cause of the constant "nothing to rewind" confirmed on a real session file: hosts interleave non-wire `custom` entries (todo HUD, …), the snapshot filter drops them, and the old `parentId` walk died at the first hole right after the leaf. `/rewind` now lists both prompts newest-first; confirming on "beta" moved the leaf (`2 entries leave`), truncated the replica, and toasted the restored draft. `/branch` opens the same picker (TUI parity). `/tree` rendered the real host tree — accent spine for the active branch, `leaf` chip, and the abandoned beta branch as non-branch rows; moving the leaf onto the abandoned beta answer resynced via reconnect and the turn came back as active. Per-message "rewind here" buttons: `:hover` fires exactly on the pointer's row (rules verified in CSSOM; this headless env reports `hover: none`, which also exercises the always-visible touch fallback at 390×844 with no overflow); clicking alpha's button dropped both turns and restored the alpha draft. New cmd `get-tree` covered by `session-host-tree.test.ts` (branch fixture with pruned-`custom` re-homing), route by `cmd.test.ts` | ✅ |
